...
export class RegisterComponent implements OnInit {
	...
	submit() {
		...
		if (this.validate(this.f)) {
			this.authService.register(this.model).subscribe(response => {
				...
				// this.router.navigate(['/' + this.authService.getUserStartPage()]);
	...
	validate(f: NgForm) {
		...
		if(objectPath.get(f, 'form.controls.regcode.errors.required')) {
			this.errors.push(this.translate.instant('AUTH.VALIDATION.REQUIRED', {name: this.translate.instant('Registrationcode')}));
		}
